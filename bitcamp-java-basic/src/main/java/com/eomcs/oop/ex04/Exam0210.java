...
public class Exam0210 {
  public static void main(String[] args) throws Exception {
    ...
    byte[] bytes = s2.getBytes("UTF-8"); // JVM은 UTF-16 2바이트 => 지정한 문자표
    // Linux/Unix : 41 42 43 ea b0 80 ea b0 81
    // Windows : 41 42 43 b0 a1 b0 b2
    for (byte b : bytes) {
    ...
    System.out.println(cal.get(Calendar.DAY_OF_WEEK)); // 요일 (일 - 1, 월 - 2, 화 -3...)
    System.out.println(cal.get(Calendar.WEEK_OF_MONTH));  // 그 달의 몇번째 주
    ...
    // int -> Ingeger, byte -> Byte short -> Short long -> Long
